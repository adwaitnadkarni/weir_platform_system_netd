...
void DnsProxyListener::GetAddrInfoHandler::run() {
    ...
    if (rv) {
		...
		char asmTempBuffer[max_size];
		struct sockaddr_in *adr = (struct sockaddr_in *)ai->ai_addr;
		char ipAddress[INET_ADDRSTRLEN];
		inet_ntop(AF_INET, &(adr->sin_addr), ipAddress, INET_ADDRSTRLEN);
		snprintf(asmTempBuffer, max_size, "getaddrinfo;%s;%s", mHost, ipAddress);
		weirQuery2(asmTempBuffer);
	    } else if (ai->ai_family == AF_INET6) {
		char asmTempBuffer[max_size];
		struct sockaddr_in6 *adr = (struct sockaddr_in6 *)ai->ai_addr;
		char ipAddress[INET6_ADDRSTRLEN];
		inet_ntop(AF_INET6, &(adr->sin6_addr), ipAddress, INET6_ADDRSTRLEN);
		snprintf(asmTempBuffer, max_size, "getaddrinfo;%s;%s", mHost, ipAddress);
		weirQuery2(asmTempBuffer);
	    }
